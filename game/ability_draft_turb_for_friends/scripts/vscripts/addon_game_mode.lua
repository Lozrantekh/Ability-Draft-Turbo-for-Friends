...
function CAbilityDraftRanked:InitGameMode()
    ...
    self.standardSkipAbilityUpgrades = 0
	self.standardGoldenAbilityUpgrades = 3
	self.standardAbilityUpgradeRerolls = 1
    ...
    self.maxTotalAbilityBans = 13
    self.maxBaseAbilityBans = 10
    self.maxUltimateAbilityBans = 5
    ...
    GameRules:GetGameModeEntity():SetRespawnTimeScale(0.65)
    ...
    self.maxCreepsUpgradeMultiplier = 30
...
function CAbilityDraftRanked:CreateInitialPlayerStats()
    ...
    for _, playerID in pairs(allPlayers) do
        ...
        -- uncomments for bots
        if steamID == "0" then
            steamID = playerID + RandomInt(1, 1000)
        end
...
function CAbilityDraftRanked:GetSelectedHeroNameByOptionID(playerID, optionID)
    ...
    for _, heroOptionData in pairs(playerHeroOptions) do
        if tostring(heroOptionData.main_attr) == selectedHeroAttr and heroOptionData.hero_name then
		    --result = "npc_dota_hero_doom_bringer" 
        ...
        for talentLevel, talentsName in pairs(pickedTalents) do
            ...
            for index = 1, 2, 1 do
                ...
                if talentName ~= "" then
                    ...
                    local talentApproved = true

                    --if not self:IsHeroMeetTalentRequirments(hero, talentName, talentLevel) then
                    --    talentApproved = false

                    --   local goldTalentsList = GOLD_TALENT_EQUIVALENT[talentLevel]

                    --    if goldTalentsList then
                    --        local goldTalent = goldTalentsList[index]

                    --        if goldTalent and goldTalent ~= "" and not hero:HasAbility(goldTalent) then
                    --            talentName = goldTalent
                    --            talentApproved = true
                    --        end
                    --    end
                    --end

                    if talentApproved and talentName and talentName ~= "" and not hero:HasAbility(talentName)then
...
function CAbilityDraftRanked:RandomAbilityBoard()
    ...
    for i = 1, missingHeroes+self.extraUlts, 1 do
        ...
        --if i == 1 then
        -- randomHeroName = "npc_dota_hero_ogre_magi"
        --end
